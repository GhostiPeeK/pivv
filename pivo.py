...
def view_profile_keyboard(viewed_user_id, viewed_username):
    ...
    builder.button(text=f"⚠️ Пожаловаться", callback_data=f"complaint_{viewed_user_id}")
...
async def view_profiles_button(message: Message, state: FSMContext):
    await state.clear()
    user_id = message.from_user.id
...
async def premium_button(message: Message, state: FSMContext):
    await state.clear()
    user_id = message.from_user.id
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await message.answer(f"🍺 ПИВЧИК", reply_markup=main_menu(user_id))
...
async def show_next_profile(message: Message, user_id: int):
    ...
    if photo:
        ...
        await message.answer(text, reply_markup=view_profile_keyboard(profile[1], profile[-1]))
...
async def show_my_profile(message: Message, user_id: int):
    ...
    if photo:
        await message.answer_photo(photo=photo, caption=text, reply_markup=profile_menu())
    else:
...
async def process_name(message: Message, state: FSMContext):
    ...
    await message.answer(f"📅 Сколько тебе лет? (от {MIN_AGE} до {MAX_AGE})")
...
async def process_age(message: Message, state: FSMContext):
    ...
    await message.answer("👤 Выбери пол:", reply_markup=builder.as_markup(resize_keyboard=True))
...
async def process_gender(message: Message, state: FSMContext):
    ...
    await message.answer("🏙 Из какого ты города?", reply_markup=get_back_keyboard())
...
async def process_about(message: Message, state: FSMContext):
    ...
    if any(x in message.text.lower() for x in forbidden):
        await message.answer(f"❌ В описании запрещены ссылки и юзернеймы!")
        return
...
async def process_photo(message: Message, state: FSMContext):
    ...
    await message.answer_photo(photo=data['photo'], caption=preview_text, reply_markup=confirm_builder.as_markup())
...
async def confirm_profile(callback: CallbackQuery, state: FSMContext):
    ...
    cursor.execute('''
        ...
        (user_id, name, age, gender, city, about, photo, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (
        user_id, data['name'], data['age'], data['gender'],
        data['city'], data['about'], data['photo'],
        datetime.now().isoformat(),
        datetime.now().isoformat()
    ))
    db.conn.commit()
    ...
    await callback.message.answer(f"🍺 ПИВЧИК", reply_markup=main_menu(user_id))
...
async def process_like(callback: CallbackQuery):
    ...
    try:
        ...
        if cursor.fetchone():
            ...
            builder = InlineKeyboardBuilder()
            if to_username:
                builder.button(text=f"📱 Написать {to_name}", url=f"https://t.me/{to_username}")
...
async def my_profile_callback(callback: CallbackQuery):
    ...
    if not profile:
        await callback.message.edit_text(f"❌ У тебя ещё нет анкеты")
        await callback.message.answer(f"🍺 ПИВЧИК", reply_markup=main_menu(user_id))
        return
...
async def view_profiles_callback(callback: CallbackQuery):
    ...
    if views_used >= limit:
        await callback.message.edit_text(f"❌ Лимит просмотров исчерпан ({limit})")
        return
